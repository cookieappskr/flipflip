...
function TypeChildren({
  ...
  if (loading) return <p className="pl-8 py-1 text-sm text-text-secondary">로딩 중...</p>;

  if (types.length === 0) {
    return <p className="pl-8 py-1 text-sm text-text-secondary">하위 유형 없음</p>;
  }
...
export default function TypeTree({ selectedTypeId, onSelectType }: TypeTreeProps) {
  const { types: rootTypes, loading } = useTypes(null);
