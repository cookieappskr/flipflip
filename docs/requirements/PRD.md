...
> **컨셉**: 세상에서 가장 쉬운 언어학습앱
> **회사**: 쿠키앱스 (CookieApps)
> **담당자**: 김무현 / [help@cookieapps.kr](mailto:help@cookieapps.kr)
...
- **인증 방식**: Google OAuth
- **접근 제어**: 화이트리스트 방식 (DB `admin_users` 테이블에 허용 이메일 관리), **초기부터 RBAC(역할 기반 접근 제어) 확장 가능하도록 설계**
- **비승인 사용자 차단**: `admin_users` 테이블에 등록되지 않은 사용자가 로그인 시도 시 "승인된 관리자가 아닙니다" 에러 메시지를 표시하고 로그인 페이지에 머물러야 함 (대시보드 접근 불가)
  - OAuth 콜백 시점에 `admin_users` 테이블 확인 → 미승인 시 세션 파기 후 로그인 페이지로 리다이렉트
  - 대시보드 진입 시 `AdminContext`에서 이중 확인 → 사용자 웹앱 등의 기존 세션으로 우회 접근 방지
- **프로필**: Google 계정의 이름/프로필 이미지 활용
- **admin_users 테이블**: role 필드 포함 (superadmin, admin, editor, viewer 등)

> **변경사항 (2026-03-22)**: 비승인 사용자 차단 요건 추가. 어드민 유저가 아닌 경우 "승인된 관리자가 아닙니다" 메시지 출력 후 로그인 페이지에 머물러야 함.
...
> **변경사항**: 전화번호 인증은 제외됨 (PO 확인 2026-03-15)
...
| **회사명**    | 쿠키앱스                                                    |
| **담당자**    | 김무현                                                     |
| **이메일**    | [help@cookieapps.kr](mailto:help@cookieapps.kr)         |
...
### 13.3 데이터 다운로드 전략 (Data Download Strategy)

> **변경사항**: 2026-03-16 — 기존 "원격 DB 접속 효율화"에서 Dexie.js(IndexedDB) 기반 로컬 DB 우선 전략으로 확장. 레벨 전체 데이터를 다운로드하되 첫 2개 스킬 우선 로드 후 나머지 백그라운드 다운로드 방식 도입.

| 시점             | 동작                                                        |
| -------------- | --------------------------------------------------------- |
| **최초 학습 시작**   | 현재 레벨 전체 데이터 다운로드 (첫 2개 스킬 우선, 나머지 백그라운드)               |
| **레벨업**        | 새 레벨 전체 데이터 다운로드 (첫 2개 스킬 우선, 나머지 백그라운드)                |
| **학습 중**       | 로컬 DB에서만 읽기/쓰기                                           |
| **당일 학습 종료**   | 학습 기록 원격 DB 일괄 저장                                        |
| **세션 종료**      | 일일 동기화 (daily sync)                                      |

첫 2개 스킬 데이터가 다운로드되면 프로그레스바가 완료되고 학습이 시작됨. 나머지 스킬 데이터는 백그라운드에서 자동 다운로드됨. IndexedDB 미지원 환경에서는 기존 온라인 모드(서버 직접 요청)로 동작함.
